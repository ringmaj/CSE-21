...
public class ScorerCombo {
	...
	// the number of innings in a regulation game (after which only tie scores
	// continue)
	// in MLB, this is 9. A smaller number will make manual testing easier.
	public static int REGULATION_NUM_INNINGS = 3;

	// assume game can never go over this number of innings (used when
	// allocating arrays)
	public static int MAX_BOX_SCORE_LENGTH = 20;

	// value in a boxScore: an indicator that a half-inning hasn't been played
	// yet
	...
	public static void readScores(Scanner scores, int[] visitorScores,
			int[] homeScores) {
	...
	public static boolean gameIsOver(int inning, int batter,
			int[] visitorScores, int[] homeScores) {
	...
	public static String result(int inning, int visitorScore, int homeScore) {
		if (visitorScore > homeScore) {
			return ("The visiting team won " + visitorScore + " to "
					+ homeScore + " in " + inning + " innings.");
		} else if (homeScore > visitorScore) {
			return ("The home team won " + homeScore + " to " + visitorScore
					+ " in " + inning + " innings.");
		} else {
